...
const ShippingStandard: React.FC = (): any => {

    const [isChecked, setChecked] = useState(true);
    const productContext: any = useContext(ProductContext);
    const {dispatch} = productContext;
    const checkBtn = (input: any): any => {
        dispatch({type: 'CHECK', payload: parseInt(input, 0)});
        setChecked(false);
    };

    const checkBtnStandard = (input: any): any => {
        dispatch({type: 'CHECK', payload: parseInt(input, 0)});
        setChecked(true);
    };
        ...
        <React.Fragment>
            ...
            <div className = "shipping__component__radio">
                ...
                {({input}: any): any => {
                    ...
                    return(
                        <label>
                        <input onClick={(): any => checkBtnStandard(input.value)}   {...input} />
            ...
            <div className="shipping__component__radio">
                ...
                {({input}: any): any => {
                    return(
                        <label>
                        <input onClick={(): any => checkBtn(parseInt(input.value, 0))}  {...input}/>
                        <span className="circle" />
                        Express Shipping
