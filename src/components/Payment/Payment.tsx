...
const Payment: React.FC = (): any => {
    ...
    return (
        <div className="payment">
            ...
            <div style={{backgroundColor: !componentPayment ? '#999' : '#000'}} className="dash-payment" />
            <h2 style={{color: !componentPayment ? '#999' : '#000'}} className="payment__heading">Payment method</h2>
            <button style={{opacity: !componentPayment ? 1 : 0}} onClick={openPayment} className={editPayment ? 'payment__heading__button' : 'editBtn'}>Edit</button>
            </div>
                ...
                <div className={componentPayment && creditPayment ? 'payment__component__credit' : 'payment__component__credit__close'}>
                    <PaymentCredit submitBtn={submitBtn}
                        setCard={setCard}
