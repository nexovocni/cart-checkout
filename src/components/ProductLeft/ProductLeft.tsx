...
const ProductLeft: React.FC<IProps> = ({ product, review }) => {
  ...
  return (
    <div
      className={`${review ? `review` : `product`}__section__left`}
      style={{
        opacity: cartState && productState ? 0.3 : 1,
        pointerEvents: cartState && productState ? "none" : "auto"
      }}
    >
      ...
      <div
        className={
          !productState
            ? `${review ? `review` : `product`}__section__left-bottom`
            : `${review ? `review` : `product`}__section__left-bottom__close`
        }
      >
        <SelectMobile
          product={product}
